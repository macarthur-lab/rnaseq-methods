...
for line in inFile:
    ...
    geneName = data[-1].split('gene_name')[-1].split(';')[0].strip()[1:-1]

    #replace the last column with a GFF formatted attributes columns
    #I added a GID attribute just to conserve all the GTF data
    row_type = data[2].lower()
    if row_type == 'gene':
        data[-1] = "ID=" + geneID + ";gene_id=" + geneID + ";gene_name=" + geneName
    elif row_type == 'transcript':
        data[-1] = "ID=" + transcriptID + ";Parent=" + geneID
    elif row_type == 'exon':
        data[-1] = "ID=exon:" + transcriptID + ";Parent=" + transcriptID
    else:
        continue
