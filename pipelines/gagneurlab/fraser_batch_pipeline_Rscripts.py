...
def get_FILTER_AND_ANNOTATE_DATA_Rscript(sample_set_label, min_reads):
    return f"""
library(FRASER)
library(data.table)
library(stringr)
...
fds = filterExpressionAndVariability(fds, minDeltaPsi=0.05, minExpressionInOneSample={min_reads}, filter=TRUE)
...
for(param_type in c("psi5", "psi3", "psiSite")) {{
    print("===============")
    fds = optimHyperParams(fds, param_type, plot=FALSE, implementation="{IMPLEMENTATION}", BPPARAM=bpparam)
...
for(param_type in c("psi5", "psi3", "psiSite")) {{
    ...
    fds = fit(fds, implementation="{IMPLEMENTATION}", q = q, type=param_type, BPPARAM=bpparam)
    gc()
...
def get_RESULTS_TABLES_Rscript(sample_set_label, delta_psi_threshold, padj_threshold):
    return f"""
library(FRASER)
library(annotables)
library(data.table)
...
ggsave(file=paste(sampleSetLabel, "_plotAberrantPerSample__padj_{padj_threshold}__dpsi_{delta_psi_threshold}.png", sep=""), g, device="png", type="cairo")
...
# filename=paste(sampleSetLabel, qLabel, "_all_results.tsv", sep="")
...
filename=paste(sampleSetLabel, qLabel, "_padj_{padj_threshold}__dpsi_{delta_psi_threshold}_results.tsv", sep="")
message("Saving ", filename)
...
def get_VOLCANO_PLOTS_Rscript(sample_set_label, delta_psi_threshold, padj_threshold):
    return f"""
library(FRASER)
library(annotables)
library(data.table)
...
for(sample_id in sample_ids) {{    
    ...
    ggsave(file=paste(sampleSetLabel, "_expression_plot_", "_padj_{padj_threshold}__dpsi_{delta_psi_threshold}_", sample_id, ".png", sep=""), p, width=12, height=8, device="png", type="cairo")
    p = plotQQ(fds, result=res2plot)
    ggsave(file=paste(sampleSetLabel, "_QQ_plot_", "_padj_{padj_threshold}__dpsi_{delta_psi_threshold}_", sample_id, ".png", sep=""), p, width=12, height=8, device="png", type="cairo")
    
    p = plotExpectedVsObservedPsi(fds, result=res2plot)
    ggsave(file=paste(sampleSetLabel, "_expectedVsObservedPsi_plot_", "_padj_{padj_threshold}__dpsi_{delta_psi_threshold}_", sample_id, ".png", sep=""), p, width=12, height=8, device="png", type="cairo")
    ...
    for(param_type in c("psi5", "psi3", "psiSite")) {{
        ...
        ggsave(file=paste(sampleSetLabel, "_volcano_", param_type, "__padj_{padj_threshold}__dpsi_{delta_psi_threshold}_", sample_id, ".png", sep=""), p, width=12, height=8, device="png", type="cairo")
    }}
}}
