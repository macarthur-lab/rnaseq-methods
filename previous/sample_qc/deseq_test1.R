...
for (data_type in c('rnaseqc_tpm', 'star_counts', 'rnaseqc_counts')) {
...
gtex_samples = gtex_sampleInfo[(gtex_sampleInfo$SMTSD %in% focused_tissue_subset) & gtex_sampleInfo$SMRIN > 6.5]$SAMPID
...
gene_name = 'XIST'
gene_id = 'ENSG00000229807'

gene = gene_names[gene_id]
gene_row = df_tpm[grepl(gene_id, df_tpm$geneId)]
gene_tpm = data.table(samples=rownames(t(as.matrix(gene_row))), tpm=t(as.matrix(gene_row)))
colnames(gene_tpm) = c('samples', 'TPM')
gene_row = subset(gene_row, select=-c(geneId))

ggplot(data=gene_row, aes(x=samples, y=TPM)) +
  geom_bar(stat="identity", fill="steelblue") +
  ggtitle(paste(gene_id, gene, sep=": ")) +
  theme(
...
samples_to_exclude = c(
  'MUN_FAM3_PARTIALMDC1A1_03_R1',
  'MUN_FAM2_TOTALMDC1A1_04_R1',
  'MUN_FAM1_CRTL_02_R1',
  'MUN_FAM1_CTRL_04_R1'
)

samples = c(colnames(df_all), nearby_GTEx_samples)
samples = samples[samples != 'geneId']
sampleInfo = data.table(sample = samples)
sampleInfo$group = NA
sampleInfo$group = ifelse(grepl('ATYPICAL', sampleInfo$sample), 'PATIENT_ATYPICAL', sampleInfo$group)
sampleInfo$group = ifelse(grepl('SIBLING', sampleInfo$sample), 'SIBLING_TOTAL', sampleInfo$group)
sampleInfo$group = ifelse(grepl('TOTALMD', sampleInfo$sample), 'TOTAL_CONTROL', sampleInfo$group)
sampleInfo$group = ifelse(grepl('PARTIALMD', sampleInfo$sample), 'PARTIAL_CONTROL', sampleInfo$group)
sampleInfo$group = ifelse(grepl('FAM1_CTRL', sampleInfo$sample), 'NORMAL_CONTROL', sampleInfo$group)
sampleInfo$group = ifelse(grepl('GTEX-', sampleInfo$sample), 'GTEX-MUSCLE', sampleInfo$group)
sampleInfo


gtex_counts = fread(
  '~/project__rnaseq/data/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_reads.gct.gz',
  select=c('Name', nearby_GTEx_samples))

finalSampleInfo = sampleInfo[!sampleInfo$sample %in% samples_to_exclude]
finalSampleInfo_names = finalSampleInfo$sample
finalSampleInfo = subset(finalSampleInfo, select=c('group'))
rownames(finalSampleInfo) = finalSampleInfo_names
finalSampleInfo$group = as.factor(finalSampleInfo$group)

#finalGtexSampleInfo = data.table(samples= groups='GTEx_muscle')

df_rnaseqc = subset(df_rnaseqc, select=c('geneId', rownames(finalSampleInfo)[finalSampleInfo$group != 'GTEX-MUSCLE']))

merged_counts = merge(df_rnaseqc, gtex_counts, by.x="geneId", by.y='Name')
merged_counts_names = merged_counts$geneId
colsMiusGeneId = colnames(merged_counts)[!colnames(merged_counts) %in% c('geneId')]
merged_counts = merged_counts[,..colsMiusGeneId]
rownames(merged_counts) = merged_counts_names

#http://www.arrayserver.com/wiki/index.php?title=TPM
dds <- DESeqDataSetFromMatrix(countData = merged_counts,
                                colData = finalSampleInfo,
                                design = ~group)
dds <- DESeq(dds)

# Levels: GTEX-MUSCLE NORMAL_CONTROL PARTIAL_CONTROL PATIENT_ATYPICAL SIBLING_TOTAL TOTAL_CONTROL
res <- results(dds, contrast=c("group", "PATIENT_ATYPICAL","SIBLING_TOTAL"))

resultsNames(dds)
resLFC = lfcShrink(dds, coef="group_SIBLING_TOTAL_vs_GTEX.MUSCLE", type="apeglm")
