...
from utils import RNASEQ_METADATA_SPREADSHEET, DATA_PATHS_WORKSHEET, BERYLS_WORKSHEET, SEQR_INFO_AND_OTHER_METADATA_WORKSHEET
...
beryls_ws_df['sample_id'] = ""
for s in beryls_ws_df['Sample ID']:
    ...
    beryls_ws_df.loc[beryls_ws_df['Sample ID'] == s, 'sample_id'] = all_samples_row.sample_id.item()

joined_df = data_paths_ws_df.merge(beryls_ws_df, on="sample_id", how="left")
joined_df2 = beryls_ws_df.merge(data_paths_ws_df, on="sample_id", how="left")

#joined_df
print("Found match for", joined_df2[~joined_df2['hg19_bam'].isnull()].shape[0], "out of", beryls_ws_df.shape[0], "rows")
print("Match not found for\n  '" + "'\n  '".join(joined_df2[joined_df2['hg19_bam'].isnull()]['Sample ID']))

print("Found match for", joined_df[~joined_df['Sex'].isnull()].shape[0], "out of", beryls_ws_df.shape[0], "rows")
print("-----------------")
...
joined_df = joined_df.rename(columns={
    "Alias":                "Alias (Beryl)",
    "Clinical Diagnosis":   "Clinical Diagnosis (Beryl)",
    "Sex":                  "Sex (Beryl)",
    "Age at muscle biopsy": "Age at muscle biopsy (Beryl)",
    "Site of biopsy":       "Site of biopsy (Beryl)",
    "Previous NGS testing": "Previous NGS testing (Beryl)",
    "Status of genetic diagnosis before RNA-seq": "Genetic diagnosis before RNA-seq (Beryl)",
})

joined_df["Notes (Beryl)"] = joined_df["Notes on genetic diagnosis status"].fillna(' ')
...
for sample_id, indivs in sample_id_to_indivs.items():
    ...
    for indiv in indivs:
        ...
        if indiv.sex:
            seqr_fields['sex (seqr)'].append(indiv.sex or "")
        if indiv.population:
    ...
    seqr_fields['sex (seqr)'] = ", ".join(sorted(set(seqr_fields['sex (seqr)']))).encode("utf-8")
